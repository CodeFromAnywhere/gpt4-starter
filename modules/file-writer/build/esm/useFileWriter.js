var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { api, queries } from "api";
import { isCtrlS, useHotkey } from "hotkeys";
import * as React from "react";
import { Div, P } from "react-with-native";
import { toast } from "react-with-native-notification";
import { WriterInput } from "writer-input";
import { useStore } from "./store";
var saveFileContentsWithContext = api.saveFileContentsWithContext;
var useGetFileContentsWithContext = queries.useGetFileContentsWithContext;
/**
 * Returns a filewriter for a project relative filePath with a function to save it
 */
export var useFileWriter = function (props) {
    var _a, _b, _c;
    var markdownModelName = props.markdownModelName, projectRelativeFilePath = props.projectRelativeFilePath, initialWriterView = props.initialWriterView, disabledMenuItems = props.disabledMenuItems, hideButtons = props.hideButtons;
    // fullPath is the project relative path
    var _d = useGetFileContentsWithContext(projectRelativeFilePath), fileContentsResult = _d.data, isLoading = _d.isLoading, refetch = _d.refetch;
    var fileContents = (_a = fileContentsResult === null || fileContentsResult === void 0 ? void 0 : fileContentsResult.result) === null || _a === void 0 ? void 0 : _a.fileContents;
    // console.log({ fileContents });
    var _e = useStore("unsavedFiles"), unsavedFiles = _e[0], setUnsavedFiles = _e[1];
    var unsavedFileContent = projectRelativeFilePath
        ? unsavedFiles[projectRelativeFilePath]
        : undefined;
    var isSaved = projectRelativeFilePath
        ? unsavedFileContent === undefined
        : true;
    var value = isSaved ? fileContents || "" : unsavedFileContent || "";
    var onChange = function (text) {
        var _a;
        if (!projectRelativeFilePath)
            return;
        setUnsavedFiles(__assign(__assign({}, unsavedFiles), (_a = {}, _a[projectRelativeFilePath] = text, _a)));
    };
    var save = function () { return __awaiter(void 0, void 0, void 0, function () {
        var apiResult, isSuccessful, result, message, newUnsavedFiles;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!projectRelativeFilePath)
                        return [2 /*return*/];
                    return [4 /*yield*/, saveFileContentsWithContext(projectRelativeFilePath, value)];
                case 1:
                    apiResult = _a.sent();
                    return [4 /*yield*/, refetch()];
                case 2:
                    _a.sent();
                    isSuccessful = apiResult.isSuccessful, result = apiResult.result, message = apiResult.message;
                    if (!(!isSuccessful || !(result === null || result === void 0 ? void 0 : result.isSuccessful))) return [3 /*break*/, 3];
                    toast({
                        title: "Error",
                        body: (result === null || result === void 0 ? void 0 : result.message) || message || "Something went wrong",
                    });
                    return [3 /*break*/, 5];
                case 3:
                    newUnsavedFiles = __assign({}, unsavedFiles);
                    // NB: for some reason, delete` doesn't let the updated store rerender the object. React doesn't see the change! The solution is to also set it to undefined first.
                    newUnsavedFiles[projectRelativeFilePath] = undefined;
                    delete unsavedFiles[projectRelativeFilePath];
                    return [4 /*yield*/, setUnsavedFiles(newUnsavedFiles)];
                case 4:
                    _a.sent();
                    _a.label = 5;
                case 5: return [2 /*return*/];
            }
        });
    }); };
    useHotkey(isCtrlS, save, [projectRelativeFilePath, value]);
    var errorMessage = !(fileContentsResult === null || fileContentsResult === void 0 ? void 0 : fileContentsResult.isSuccessful)
        ? fileContentsResult === null || fileContentsResult === void 0 ? void 0 : fileContentsResult.message
        : !((_b = fileContentsResult === null || fileContentsResult === void 0 ? void 0 : fileContentsResult.result) === null || _b === void 0 ? void 0 : _b.isSuccessful)
            ? (_c = fileContentsResult === null || fileContentsResult === void 0 ? void 0 : fileContentsResult.result) === null || _c === void 0 ? void 0 : _c.message
            : null;
    var renderFileWriter = function () {
        return (React.createElement(Div, { className: "flex flex-1 flex-col h-full" }, errorMessage ? (React.createElement(P, null, errorMessage)) : (React.createElement(WriterInput, { key: projectRelativeFilePath, isLoading: isLoading, onChange: onChange, value: value, projectRelativeFilePath: projectRelativeFilePath, reload: refetch, save: save, hideButtons: hideButtons, initialWriterView: initialWriterView, isSaved: isSaved, markdownModelName: markdownModelName, disabledMenuItems: disabledMenuItems }))));
    };
    return {
        renderFileWriter: renderFileWriter,
        save: save,
        isSaved: isSaved,
    };
};
//# sourceMappingURL=useFileWriter.js.map